...
integer HandleHTTPResponse( key id, integer status, string body ){
    if ( id == RequestId )
    {
        if ( status == 499 )
            ...
            list results = llCSV2List( body );
            integer N = llGetListLength( results );
            integer i;

            if( N >= 3 ){
                integer index = (integer)llList2String( results, 0 );
                Total =  (integer)llList2String( results, 1 );
                integer duration = (integer)llList2String( results, 2 );
                string musicURL = llList2String( results, 3 );
                for( i = 4; i<N; i++ )
                    MusicURL += "," + llList2String( results, i );
...
state Pausing {
    ...
    http_response(key id, integer status, list meta, string body){
        // let it drop on the floor -- we'll resume when we return to PlayList
    }
