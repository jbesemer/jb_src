...
URL = "http://cascade-sys.com/cgi-bin/visitor.cgi"
...
class MyDb:
	...
	def Record( self, ts, sensor, action, name ):
		"""record a visitor's arrival or departure"""

		return self.Execute( 
			"""Insert into log (timestamp,timecode,sensor,action,name) """
			+ """VALUES( '%s', '%s', '%s', '%s', '%s' );"""
				% ( ParseDate( ts ), ts, sensor, action, name ))
...
#					self.DateKeyL + "='%s'" % self.GetDate(),
	...
	def ComputeDuration( self, arrival, departure ):
		try:
			a = self.Time2Seconds( arrival )
			d = self.Time2Seconds( departure )
		except:
			return "?"
...
class MyFieldStorage( cgi.FieldStorage ):
	...
	def IsAddRecord( self ):
		return ( self.has_key( 'time' )
			and self.has_key( 'sensor' )
			and self.has_key( 'action' )
			and self.has_key( 'name' ) )
			
	def RecordArguments( self ):
	...
	def GetDate( self ):
		if self.has_key( 'date' ):
			return ParseDate( self[ 'date' ].value )
		else:
			return ""

	def IsDebugMode( self ): return self.has_key( "debug" ) and self[ 'debug' ]
...
if __name__ == "__main__":
	...
	try:
		...
		if Form.IsAddRecord():
			Db.Record( *Form.RecordArguments())
			print "OK"
		else:
			Db.ShowHistory( Form.GetDate())
